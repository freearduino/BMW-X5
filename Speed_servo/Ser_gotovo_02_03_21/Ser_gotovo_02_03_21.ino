...
void loop()
{
  ...
  if (!digitalRead(ruchnik))
  {
    ...
    if (!digitalRead(ruchnik))
    {
      ...
      PanelServo1.attach(6);
      digitalWrite(PowerSer3, HIGH);
      PanelServo1.write(10); //Угол открытия 
      delay(500); //Вращаем определенное время. Крышка спидометра
      digitalWrite(PowerSer3, LOW);
      PanelServo1.detach();
      delay(50);
      //Крышка поехала вниз
  ...
  else
  {
    ...
    if (digitalRead(con2) == HIGH)
    {
      ...
      PanelServo1.attach(6);
      digitalWrite(PowerSer3, HIGH);
      PanelServo1.write(150); //Угол закрытия
      delay(500); //Вращаем определенное время. Время открытия крышки спидометра
      digitalWrite(PowerSer3, LOW);
      PanelServo1.detach();
      delay(50);
      //Возможно тут нужно будет поработать сервами , которые вращаются в разные стороны
