...
void loop()
{
  ...
  if (!digitalRead(ruchnik))
  {
    ...
    if (!digitalRead(ruchnik))
    {
      ...
      digitalWrite(PowerSer3, HIGH);
      PanelServo1.write(0);
      delay(400); //Вращаем определенное время. Крышка спидометра поднимается
      digitalWrite(PowerSer3, LOW);
      PanelServo1.detach();
      delay(50);
      //Крышка поехала вниз
  ...
  else
  {
    ...
    {
      delay(50);
      //Поехали пока не нажали концевик
      while (digitalRead(con2) == HIGH)
      {
        //Запускаем серву
        PanelServo2.attach(9);
...
#endif
      delay(50);
      //Крышка спидометра закрывается
      PanelServo1.attach(6);
      digitalWrite(PowerSer3, HIGH);
      PanelServo1.write(180);
      delay(450); //Вращаем определенное время. Время открытия крышки спидометра. Двигатели постоянного вращения.
      digitalWrite(PowerSer3, LOW);
      PanelServo1.detach();
      delay(50);
      //Возможно тут нужно будет поработать сервами , которые вращаются в разные стороны
      //Движенпие панели приборов
      BoxCover1.attach(3);
      BoxCover2.attach(5);
      digitalWrite(PowerSer1, HIGH);
      digitalWrite(PowerSer2, HIGH);
